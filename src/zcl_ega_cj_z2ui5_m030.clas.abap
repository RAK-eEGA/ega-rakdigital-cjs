...
private section.
  ...
  methods CHK_OPEN_POA_CASE
    importing
      value(BP) type /IWBEP/T_COD_SELECT_OPTIONS optional
    returning
      value(OPEN_CASES) type ZTT_OPEN_POA_CASES .
  methods GET_LR
    importing
...
*        AND valid_date_to GE @sy-datum.
  ...
  METHOD init_journey.
    ...
    <search>-id = 'LESSOR'.
    APPEND INITIAL LINE TO <search>-methods ASSIGNING FIELD-SYMBOL(<method>).
    <method>-method        = 'EMIRATE_ID'.
    <method>-description   = 'Emirates ID'.
    <method>-subject       = 'Emirates ID'.
    <method>-mask          = '999-9999-9999999-9'.
    <method>-input         = abap_true.
    <method>-birth_date    = abap_true.
    <method>-nationality   = abap_true.
    APPEND INITIAL LINE TO <search>-methods ASSIGNING <method>.
    <method>-method        = 'PASSPORT'.
    <method>-description   = 'Passport(Non EID Holder only)'.
    <method>-subject       = 'Passport Number'.
    <method>-input         = abap_true.
    <method>-birth_date    = abap_true.
    <method>-passport_type = abap_true.
    <method>-nationality   = abap_true.
    APPEND INITIAL LINE TO <search>-methods ASSIGNING <method>.
    <method>-method        = 'UNIFIED_ID'.
    <method>-description   = 'Unified ID(Non EID Holder only)'.
    <method>-subject       = 'Unified ID'.
    <method>-input         = abap_true.
    <method>-birth_date    = abap_true.
    <method>-nationality   = abap_true.
    APPEND INITIAL LINE TO <search>-methods ASSIGNING <method>.
    <method>-method        = 'TRADE_LICENSE'.
    <method>-description   = 'Trade License Number'.
    <method>-subject       = 'Trade License Number'.
    <method>-input         = abap_true.
...
*    DATA(label_uploader) = label_uploader_hbox->label( text = get_text_by_id( 'LABOUR_HOUSING_REPORT' ) class = 'Body_1_2 color-gray7' ).
    ...
    CASE client->get( )-event.
      WHEN 'SAVE'.
        CASE me->get_current_screen( ).
          WHEN 'NNTC_1_2'.
            READ TABLE gs_data-properties INTO gs_data-selection-property WITH KEY selected = abap_true.
            IF sy-subrc NE 0.
              CLEAR: gs_data-selection-property.
            ENDIF.
        ENDCASE.
      ...
      WHEN 'BACK'.
        me->step_forward( control = me direction = '-' ).
      WHEN 'SEARCH'.
        me->raksearch_get_data( client->get_event_arg( 1 ) ).
        me->step_forward( control = me direction = '=' ).
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD lease_properties.
    ...
    DATA:lr_role      TYPE /iwbep/t_cod_select_options,
         ...
         lt_role      TYPE /iwbep/t_cod_select_options,
         lt_partner   TYPE /iwbep/t_cod_select_options,
...
*    ENDLOOP.
    ...
    SHIFT gs_data-year LEFT DELETING LEADING '0'.
    IF gs_data-year < '2014'.
      DATA(lv_ownind) = abap_true.
    ELSEIF gs_data-year = '2014' AND gs_data-deed LE 3000.
      lv_ownind = abap_true.
    ELSE.
      lv_ownind = abap_false.
    ENDIF.

    DATA(lv_val) = gs_data-deed+0(1).
    gs_data-deed+0(1) = SWITCH char01( owntyp WHEN 'P' THEN '0' WHEN 'T' THEN '9' ELSE lv_val ).
*    deed = |{ deed ALPHA = IN }|.
    gs_data-year = |{ gs_data-year ALPHA = IN }|.

    IF app_type = 'POA'.
      CLEAR role.
    ENDIF.

    partnerrole = COND #( WHEN role EQ 'LESSOR' THEN 'ZLESOR'
                          WHEN role EQ 'LESSEE' THEN 'ZLESEE'
                          WHEN role EQ 'AGENT' THEN 'ZLESOR'
                          WHEN role EQ 'REP' THEN 'ZLESEE'
                          ).

    " get the representative partners in case of agent role. " added on 17.03.2026
*    IF role = 'AGENT' OR role = 'LESSOR'." commented by sherine to allow representatives on 19.08.2026
    IF role = 'AGENT' OR role = 'LESSOR' OR app_type = 'POA'." added by sherine to allow representatives on 19.08.2026
      SELECT partner1 FROM but050
      ...
      IF sy-subrc IS INITIAL.
        LOOP AT lt_comp INTO DATA(ls_comp).
          lt_partner = VALUE #( BASE lt_partner ( sign = 'I' option = 'EQ' low = ls_comp-partner1 ) ).
        ENDLOOP.
      ENDIF.
    ENDIF.
    lt_partner = VALUE #( BASE lt_partner ( sign = 'I' option = 'EQ' low = gs_data-partner ) ).
* end of changes on 17.03.2026


    IF app_type = 'POA'.
      partnerrole = 'TR0800'.
*      lt_role = VALUE #( BASE lt_role ( sign = 'I' option = 'EQ' low = 'ZLESOR' ) ).
*      DATA(poacases) = chk_open_poa_case( bp = xpartner ). " commented by sherine to allow representatives on 19.08.2026
      DATA(poacases) = chk_open_poa_case( bp = lt_partner ). " added by sherine to allow representatives on 19.08.2026
    ENDIF.

    lt_role = VALUE #( BASE lt_role ( sign = 'I' option = 'EQ' low = partnerrole ) ).
...
**      lo_obj = NEW zcl_ega_mun_cj_odata_api_v2( partner = partner role = CONV #( partnerrole ) deed = deed year = year ).
*      lo_obj = NEW zcl_ega_mun_cj_odata_api_v2( partner = partner role = lt_role deed = deed year = year ).
*
*      DATA(mt_properties_inv) = lo_obj->properties.
*      DATA(parcel_inv) = lo_obj->get_pl_header( ). "Get associated parcel
*      DATA(ao_inv) = lo_obj->get_ao_header( ).     "Get associated AO
*      DATA(location_inv) = lo_obj->get_location( )."Get associated location
*      DATA(status_inv) = lo_obj->get_status( ).    "Get associated status
*
*      APPEND LINES OF mt_properties_inv TO mt_properties.
*      APPEND LINES OF parcel_inv TO parcel.
*      APPEND LINES OF ao_inv TO ao.
*      APPEND LINES OF location_inv TO location.
*      APPEND LINES OF status_inv TO status.
    ENDIF.

*    DATA(lo_obj) = NEW zcl_ega_mun_cj_odata_api_v2( partner = partner role = CONV #( partnerrole ) deed = deed year = year ).
*    DATA(lo_obj) = NEW zcl_ega_mun_cj_odata_api_v2( partner = partner role = lt_role deed = deed year = year )." commented on 17.03.2026
    DATA(lo_obj) = NEW zcl_ega_mun_cj_odata_api_v2( partner = lt_partner role = lt_role deed = gs_data-deed year = gs_data-year )." added for rep on 17.03.2026
...
*    DELETE mt_properties WHERE role NOT IN lt_role.

    et_entityset = VALUE #( BASE et_entityset FOR wa IN parcel (  parcelid = wa-plno
                                                                  parceldesc = wa-xpl
                                                                  locationdesc = wa-xpl
                                                                  landuse = COND #( WHEN VALUE #( mt_properties[ intreno = wa-intreno ]-ownershpmthd OPTIONAL ) IS INITIAL THEN wa-xfixfitcharact ELSE
                                                                                          VALUE #( mt_properties[ intreno = wa-intreno ]-ownershpmthd OPTIONAL ) )
                                                                  parcelvalidfrom = COND #( WHEN wa-validfrom IS NOT INITIAL THEN ctt( i_d = wa-validfrom i_t = '000000' ) )
                                                                  parcelvalidto = COND #( WHEN VALUE #( mt_properties[ intreno = wa-intreno ]-is_grant OPTIONAL ) EQ 'X'
                                                                  THEN
                                                                  COND #( WHEN VALUE #( mt_properties[ intreno = wa-intreno ]-validto OPTIONAL ) IS NOT INITIAL
                                                                  THEN ctt( i_d = VALUE #( mt_properties[ intreno = wa-intreno ]-validto OPTIONAL ) i_t = '000000' ) )
                                                                  ELSE
                                                                  ...
                                                                  favourite = VALUE #( mt_properties[ intreno = wa-intreno ]-favourite OPTIONAL )
                                                                  address = parcel_address( parcel = wa-plno )
                                                                  ownershpmthd = VALUE #( mt_properties[ intreno = wa-intreno ]-ownershpmthd OPTIONAL )
                                                                  ownershiptype =  VALUE #( mt_properties[ intreno = wa-intreno ]-ownmethod OPTIONAL )
                                                                  granttype =  VALUE #( mt_properties[ intreno = wa-intreno ]-granttype OPTIONAL )
                                                                  departmentcode = 'MUN'
...
*        ENDIF.
      ENDIF.
      IF role = 'LESSEE' AND gs_data-deed IS NOT INITIAL AND gs_data-year IS NOT INITIAL.
        vol = gs_data-deed.
        pgno = gs_data-year.
      ENDIF.
      ...
      IF app_type = 'POA'.
        DATA(ident) = VALUE #( poacases[ pagno = <fs_entity>-titledeed volno = <fs_entity>-titledeedyear ]-ident OPTIONAL ).
        DATA(stat) = VALUE #( poacases[ pagno = <fs_entity>-titledeed volno = <fs_entity>-titledeedyear ]-status OPTIONAL ).
        <fs_entity>-availableunits = COND #( WHEN ident = abap_true THEN '0' ELSE <fs_entity>-availableunits ).
        IF stat > '10' AND stat < '70'.
          <fs_entity>-contractstatus =  '1'."COND #( WHEN ident = abap_true THEN '2'  ELSE  '1' )."02-Active Delegation 01-DelgationInProgress "
        ELSEIF stat <= '10'.
          <fs_entity>-contractstatus = ' '.
        ELSE.
          <fs_entity>-contractstatus = '2'.
        ENDIF.


        <fs_entity>-contractstatusdesc = COND #( WHEN <fs_entity>-contractstatus = '1' THEN TEXT-014 WHEN <fs_entity>-contractstatus = '2' THEN TEXT-015  WHEN
                                                       <fs_entity>-contractstatus = ' ' THEN ' ')."COND #( WHEN ident = abap_true THEN TEXT-015 ELSE '' )."Text-015-Active Delegation
*Text-014-DelgationInProgress
      ENDIF.
...
*          <fs_property>-ownmethod = ownmthd.
      ...
      ENDIF.
      IF role = 'LESSEE' AND gs_data-deed IS NOT INITIAL AND gs_data-year IS NOT INITIAL AND lines( et_entityset ) > 1.
    ...
    LOOP AT et_properties ASSIGNING FIELD-SYMBOL(<property>).
  ...
  METHOD nntc_1_2.
    ...
    IF ms_pagination IS INITIAL.
      ...
      ms_pagination-total_page_count = ( ms_pagination-total_lines - ( ms_pagination-total_lines MOD ms_pagination-lines_on_page ) ) / ms_pagination-lines_on_page.
...
*                             id          = 'searchField'
    ...
    DATA(building_image) = building_image_hbox->hbox( class = 'imageBuildind sapUiTinyMarginEnd p-img-top' visible = '{= ${path : ''AOTYPE'',formatter:''.AOTypeImage''} === ''building''}' ).
    DATA(no_building_image) = building_image_hbox->hbox( class = 'imageNoBuildind sapUiTinyMarginEnd p-img-top' visible = '{= ${path : ''AOTYPE'',formatter:''.AOTypeImage''} === ''noBuilding''}' ).
    DATA(unit_image) = building_image_hbox->hbox( class = 'imageUnit sapUiTinyMarginEnd p-img-top' visible = '{= ${path : ''AOTYPE'',formatter:''.AOTypeImage''} === ''unit''}' ).
...
*        class = 'sapUiTinyMarginEnd p-img-top' ).
  ...
  METHOD set_context.
    ...
    IF gs_data-selection-property IS NOT INITIAL.
      CASE gs_data-selection-context.
        WHEN '3' OR '4' OR '5'.
          CASE gs_data-selection-property-aotype.
            WHEN '10BU' OR '10BU'.
              gs_data-selection-context = '3'.
            WHEN ''.
              gs_data-selection-context = '5'.
            WHEN OTHERS.
              gs_data-selection-context = '4'.
          ENDCASE.
        WHEN '7' OR '8' OR '9'.
          CASE gs_data-selection-property-aotype.
            WHEN '10BU' OR '10BU'.
              gs_data-selection-context = '7'.
            WHEN ''.
              gs_data-selection-context = '9'.
            WHEN OTHERS.
              gs_data-selection-context = '8'.
          ENDCASE.
      ENDCASE.
    ENDIF.
...
'}' &&
'.imageNoBuildind{' &&
' background-image: url(' && no_building && ');' &&
'    background-repeat: no-repeat;' &&
'    background-size: cover;' &&
'    width: 16px;' &&
'    height: 16px;   ' &&
...
'       aFilters.push(new sap.ui.model.Filter("INDEX", "EQ", 0));' && |\n| &&
'     }' && |\n| &&
'     oList.getBinding("items").filter(aFilters); ' && |\n| &&
'   }'.
    me->add_function( ev_function ).
    ev_function =
'AOTypeImage: function (AOType) {' && |\n| &&
'     // 10 BU for Building , 20FL for the floor, 30UN, 30IU FOR untis , and in parcel there is no codes' && |\n| &&
'     if (AOType.includes("30") || AOType.includes("Unit") || AOType.includes("وحدة")) {' && |\n| &&
'       return "unit";' && |\n| &&
'     } else if (AOType.includes("10")) {' && |\n| &&
'       return "building";' && |\n| &&
'     } else {' && |\n| &&
'       return "noBuilding";' && |\n| &&
'     }' && |\n| &&
'   }'.
  ...
  METHOD nntc_1_4.
    ...
    me->raksearch( io_parent = part2 search_id = 'DEED' ).

    DATA(lessor_separator) = part2->hbox( class = 'sessions_card_hSeparator sapUiMediumMarginTopBottom' ).
    DATA(lessor_details_title) = part2->label( text = get_text_by_id( 'LESSOR_DETAILS' ) class = 'H3_1 color-dark-blue' ).

    me->raksearch( io_parent = part2 search_id = 'LESSOR'  ).
    IF gs_data-selection-rb_3rd EQ abap_true.
      DATA(lessee_cont) = part2->vbox( ).
      DATA(lessee_separator) = lessee_cont->hbox( class = 'sessions_card_hSeparator sapUiMediumMarginTopBottom' ).
      DATA(lessee_details_title) = lessee_cont->label( text = get_text_by_id( 'LESSEE_DETAILS' ) class = 'H3_1 color-dark-blue' ).
      me->raksearch( io_parent = lessee_cont search_id = 'LESSEE' ).
    ENDIF.
...
'}    '.
  ...
  METHOD chk_open_poa_case.
    TYPES: BEGIN OF ty_case,
             case_guid    TYPE scmg_case_guid,
             bo_id        TYPE zde_ega_object_id,
             stat_orderno TYPE scmg_t_case_attr-stat_orderno,
           END OF ty_case.

    DATA: volno      TYPE relmlrvolumeno,
          pagno      TYPE relmlrpageno,
          lrfull(70) ,
          new        TYPE TABLE OF ty_case,
          can        TYPE TABLE OF ty_case,
          related    TYPE TABLE OF ty_case.


    SELECT a~case_guid, b~poa_type
      FROM zdt_ega_cs_2_bo AS a
      INNER JOIN zdt_ega_caat_own AS b
      ON a~case_guid = b~case_guid
      INTO TABLE @DATA(cases)
*      WHERE a~bo_id = @bp " commented by sherine to allow representatives on 19.08.2026
      WHERE a~bo_id IN @bp " added by sherine to allow representatives on 19.08.2026
        AND a~case_type = 'ZL50'
        AND a~sps_id = 'ZEGA_SPS_BOR_BUS1006_APPL'.

    LOOP AT cases INTO DATA(case1).
      IF case1-poa_type = '1'.
        APPEND VALUE #( case_guid = case1-case_guid ) TO new.
      ELSEIF case1-poa_type = '2'.
        APPEND VALUE #( case_guid = case1-case_guid ) TO can.
      ENDIF.
    ENDLOOP.

    IF can[] IS NOT INITIAL.
      SELECT zdt_ega_cs_2_bo~case_guid, bo_id, stat_orderno
        FROM zdt_ega_cs_2_bo INNER JOIN
        scmg_t_case_attr ON scmg_t_case_attr~case_guid = zdt_ega_cs_2_bo~case_guid
        INTO TABLE @related
        FOR ALL ENTRIES IN @can
        WHERE zdt_ega_cs_2_bo~case_guid = @can-case_guid
          AND bo_type = 'SCASE'
          AND sps_id = 'ZEGA_SPS_CASE_ZL50'.
    ENDIF.

    IF new[] IS NOT INITIAL.
      SELECT a~case_guid, a~bo_id, b~create_time, c~poa_type, stat_orderno FROM zdt_ega_cs_2_bo AS a
        INNER JOIN scmg_t_case_attr AS b
        ON a~case_guid = b~case_guid
        INNER JOIN zdt_ega_caat_own AS c
        ON a~case_guid = c~case_guid
        INTO TABLE @DATA(lrs)
        FOR ALL ENTRIES IN @new
        WHERE a~case_guid = @new-case_guid
          AND bo_type = 'BUS1510'
          AND sps_id = 'ZEGA_SPS_BOR_BUS1510'
        AND ( b~stat_orderno <> '80' AND b~stat_orderno <> '88' ).
    ENDIF.

    SORT lrs BY bo_id ASCENDING create_time DESCENDING.
    DELETE ADJACENT DUPLICATES FROM lrs COMPARING bo_id.

    LOOP AT lrs INTO DATA(lr).
      APPEND INITIAL LINE TO open_cases REFERENCE INTO DATA(lo).
      DATA(rel) = VALUE #( related[ bo_id = lr-case_guid ]-bo_id OPTIONAL ).
      DATA(status) =  VALUE #( related[ bo_id = lr-case_guid ]-stat_orderno OPTIONAL ).
      IF rel IS NOT INITIAL.
        lo->ident   = COND #( WHEN status < 70 AND status > 10 THEN abap_true ELSE abap_false )."COND #( WHEN rel IS INITIAL THEN abap_true ELSE abap_false ).
        lo->status = status.
      ELSE.
        IF lr-stat_orderno > 10 AND lr-stat_orderno < 70 .
          lo->ident = abap_true.
        ELSEIF lr-stat_orderno = 70.
          lo->ident = abap_true.
        ENDIF.
      ENDIF.
      lo->lrfull  = lr-bo_id.
      lo->pagno   = lr-bo_id+6(5).
      lo->volno   = lr-bo_id+21(15).
      lo->status  = lr-stat_orderno.
    ENDLOOP.
  ENDMETHOD.
  ...
  'SAVEDRAFTHOMEPOPUP' ) ).
    ...
    DATA(delete) = card_header_end->button( id = 'DELETE' text = get_text_by_id( 'DELETE' ) class = 'RAKEGA-cardheader-topbtn RAKEGA-hide-in-mobile' icon = 'sap-icon://icomoon/Delete' press = client->_event( 'DELETE' ) ).
    DATA(delete_icononly) = card_header_end->button( id = 'DELETE_ICONONLY' class = 'RAKEGA-cardheader-topbtn RAKEGA-hide-in-desktop' icon = 'sap-icon://icomoon/Delete' press = client->_event( 'DELETE' ) ).
    DATA(description_panel) = card_top->vbox( class = 'RAKEGA-description-m-panel' alignitems = 'Start' ).
    DATA(description_title_in_panel) = description_panel->label( text = get_text_by_id( 'GENINFOHEADER' ) class = 'Body_1_3 color-gray5' ).
    DATA(context_hbox_desktop) = card_top->hbox( class = 'RAKEGA-context-hbox-desktop' ).
    DATA(context1_d_cont) = context_hbox_desktop->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx1_d_label) = context1_d_cont->label( text = get_text_by_id( 'CONTRACT' ) class = 'Body_1_3 color-gray7' ).
    DATA(ctx1_d_value) = context1_d_cont->label( text = '{CONTRACT_ID}' class = 'Body_1_3 color-gray7' ).
    DATA(context1_d_separator) = context_hbox_desktop->vbox( class = 'RAKEGA-context-separator' ).
    ...
    DATA(ctx4_d_label) = context4_d_cont->label( text = get_text_by_id( 'TEN_BUILDINGID' ) class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX4_D_VALUE): VALUE='1234' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx4_d_value) = context4_d_cont->label( text = '{BUILDING_ID1}' class = 'Body_1_3 color-gray7' ).
    DATA(context4_d_separator) = context_hbox_desktop->vbox( class = 'RAKEGA-context-separator' ).
    DATA(context2_d_cont) = context_hbox_desktop->hbox( class = 'RAKEGA-context-cont' ).
    " TODO(CTX2_D_LABEL): no VALUE and no TECHNICAL_NAME in source - binding unknown
    DATA(ctx2_d_label) = context2_d_cont->label( text = '' class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX2_D_VALUE): VALUE='Residential' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx2_d_value) = context2_d_cont->label( text = '{BUILDING_TYPE1}' class = 'Body_1_3 color-gray7' ).
    DATA(context2_d_separator) = context_hbox_desktop->vbox( class = 'RAKEGA-context-separator' ).
    DATA(context5_d_cont) = context_hbox_desktop->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx5_d_label) = context5_d_cont->label( text = get_text_by_id( 'FLOOR_NO' ) class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX5_D_VALUE): VALUE='1' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    ...
    " AMBIGUOUS(CTX7_D_VALUE): VALUE='2024-08-13T14:44:07Z' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx7_d_value) = context7_d_cont->label( text = '{EXPIRED}' class = 'Body_1_3' ).
    DATA(context_m_collapsed_hbox) = card_top->hbox( class = 'RAKEGA-context-m-collapsed-hbox' ).
    DATA(context1_mc_cont) = context_m_collapsed_hbox->hbox( class = 'RAKEGA-context-cont-collapsed' ).
    DATA(ctx1_mc_label) = context1_mc_cont->label( text = get_text_by_id( 'TEN_VILLAID' ) class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX1_MC_VALUE): VALUE='123456' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx1_mc_value) = context1_mc_cont->label( text = '{VILA_ID}' class = 'Body_1_3 color-gray7' ).
    DATA(context11_mc_cont) = context_m_collapsed_hbox->hbox( class = 'RAKEGA-context-cont-collapsed' ).
    DATA(ctx11_mc_label) = context11_mc_cont->label( text = get_text_by_id( 'TEN_BUILDINGID' ) class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX11_MC_VALUE): VALUE='1234' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx11_mc_value) = context11_mc_cont->label( text = '{BUILDING_ID2}' class = 'Body_1_3 color-gray7' ).
    DATA(ctx_more) = context_m_collapsed_hbox->button( id = 'CTX_MORE' text = get_text_by_id( 'MORE_DETAILS' ) class = 'RAKEGA-context-m-button' type = 'Transparent' ).
    " TODO(CTX_MORE): no EVENT in source - this button drives a client-side show/hide chain instead (UI_FIELD_LOGICS): CONTEXT_M_COLLAPSED_HBOX-V-F,CONTEXT_M_EXPANDED_VBOX-V-T
    DATA(context_m_expanded_vbox) = card_top->vbox( class = 'RAKEGA-context-m-expanded-vbox' ).
    DATA(context1_me_cont) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx1_me_label) = context1_me_cont->label( text = get_text_by_id( 'CONTRACT' ) class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX1_ME_VALUE): VALUE='12345678' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx1_me_value) = context1_me_cont->label( text = '{CONTRACT_ID}' class = 'Body_1_3 color-gray7' ).
    ...
    DATA(ctx4_me_label) = context4_me_cont->label( text = get_text_by_id( 'TEN_BUILDINGID' ) class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX4_ME_VALUE): VALUE='1234' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx4_me_value) = context4_me_cont->label( text = '{BUILDING_ID3}' class = 'Body_1_3 color-gray7' ).
    DATA(context2_me_cont) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-cont' ).
    " TODO(CTX2_ME_LABEL): no VALUE and no TECHNICAL_NAME in source - binding unknown
    DATA(ctx2_me_label) = context2_me_cont->label( text = '' class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX2_ME_VALUE): VALUE='Residential' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx2_me_value) = context2_me_cont->label( text = '{BUILDING_TYPE2}' class = 'Body_1_3 color-gray7' ).
    DATA(context5_me_cont) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx5_me_label) = context5_me_cont->label( text = get_text_by_id( 'FLOOR_NO' ) class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX5_ME_VALUE): VALUE='1' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx5_me_value) = context5_me_cont->label( text = '{FLOOR_NUMBER}' class = 'Body_1_3 color-gray7' ).
    ...
    DATA(ctx_less) = ctx_less_hbox->button( id = 'CTX_LESS' text = get_text_by_id( 'LESS_DETAILS' ) class = 'RAKEGA-context-m-button' type = 'Transparent' ).
    " TODO(CTX_LESS): no EVENT in source - this button drives a client-side show/hide chain instead (UI_FIELD_LOGICS): CONTEXT_M_COLLAPSED_HBOX-V-T,CONTEXT_M_EXPANDED_VBOX-V-F
    DATA(description_details_cont) = card_top->vbox( class = 'RAKEGA-card-descriptioncont RAKEGA-hide-in-mobile sapUiSmallMarginTop' alignitems = 'Center' ).
    DATA(description) = description_details_cont->label( text = get_text_by_id( 'DESCRIPTION_RA_SELECTPARCELORGRANTED_1_1' ) class = 'font1 weight400 color-gray6' ).
    DATA(description_details_panel) = card_top->vbox( class = 'RAKEGA-hide-in-desktop sapUiTinyMarginTop' alignitems = 'Start' ).
    DATA(journey_description_mobile) = description_details_panel->text_area( value = get_text_by_id( 'DESCRIPTION_RA_SELECTPARCELORGRANTED_1_1' ) maxlength = '65' class = 'Body_1_3 color-gray7 RAKEGA-lobby-ex-text' ).
    DATA(part2) = card->vbox( class = 'RAKEGA-part2' ).
    DATA(footer) = firstcontainer->hbox( class = 'RAKEGA-footer' ).
    DATA(buttonback) = footer->button( id = 'BUTTONBACK' text = get_text_by_id( 'BACK_BUTTON' ) class = 'regularBTN_with_border' icon = 'sap-icon://icomoon/Left' press = client->_event( 'BACK' ) ).
  ...
  'SAVEDRAFTHOMEPOPUP' ) ).
    ...
    DATA(context_hbox_desktop) = card_top->hbox( class = 'RAKEGA-context-hbox-desktop' ).
    DATA(context1_d_cont) = context_hbox_desktop->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx1_d_label) = context1_d_cont->label( text = get_text_by_id( 'TEN_TITLEDEED' ) class = 'Body_1_3 color-gray7' ).
    DATA(ctx1_d_value) = context1_d_cont->label( text = '{TITLE_DEED1}' class = 'Body_1_3 color-gray7' ).
    DATA(context1_d_separator) = context_hbox_desktop->vbox( class = 'RAKEGA-context-separator' ).
    DATA(context2_d_cont) = context_hbox_desktop->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx2_d_label) = context2_d_cont->label( text = get_text_by_id( 'TEN_BUILDINGID' ) class = 'Body_1_3 color-gray7' ).
    DATA(ctx2_d_value) = context2_d_cont->label( text = '{BUILDING_ID}' class = 'Body_1_3 color-gray7' ).
    DATA(context2_d_separator) = context_hbox_desktop->vbox( class = 'RAKEGA-context-separator' ).
    DATA(context3_d_cont) = context_hbox_desktop->hbox( class = 'RAKEGA-context-cont' ).
    " TODO(CTX3_D_LABEL): no VALUE and no TECHNICAL_NAME in source - binding unknown
    DATA(ctx3_d_label) = context3_d_cont->label( text = '' class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX3_D_VALUE): VALUE='Residential' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx3_d_value) = context3_d_cont->label( text = '{BUILDING_TYPE}' class = 'Body_1_3 color-gray7' ).
    DATA(context3_d_separator) = context_hbox_desktop->vbox( class = 'RAKEGA-context-separator' ).
    DATA(context4_d_cont) = context_hbox_desktop->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx4_d_value) = context4_d_cont->label( text = '{FLOORS}' class = 'Body_1_3 color-gray7' ).
    DATA(ctx4_d_label) = context4_d_cont->label( text = get_text_by_id( 'TEN_FLOORS' ) class = 'Body_1_3 color-gray7' ).
    DATA(context4_d_separator) = context_hbox_desktop->vbox( class = 'RAKEGA-context-separator' ).
    DATA(context5_d_cont) = context_hbox_desktop->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx5_d_value) = context5_d_cont->label( text = '{UNITS}' class = 'Body_1_3 color-gray7' ).
    DATA(ctx5_d_label) = context5_d_cont->label( text = get_text_by_id( 'TEN_UNITS' ) class = 'Body_1_3 color-gray7' ).
    DATA(context_m_collapsed_hbox) = card_top->hbox( class = 'RAKEGA-context-m-collapsed-hbox' ).
    DATA(context1_mc_cont) = context_m_collapsed_hbox->hbox( class = 'RAKEGA-context-cont-collapsed' ).
    DATA(ctx1_mc_label) = context1_mc_cont->label( text = get_text_by_id( 'TEN_TITLEDEED' ) class = 'Body_1_3 color-gray7' ).
    DATA(ctx1_mc_value) = context1_mc_cont->label( text = '{TITLE_DEED2}' class = 'Body_1_3 color-gray7' ).
    DATA(ctx_more) = context_m_collapsed_hbox->button( id = 'CTX_MORE' text = get_text_by_id( 'MORE_DETAILS' ) class = 'RAKEGA-context-m-button' type = 'Transparent' ).
    " TODO(CTX_MORE): no EVENT in source - this button drives a client-side show/hide chain instead (UI_FIELD_LOGICS): CONTEXT_M_COLLAPSED_HBOX-V-F,CONTEXT_M_EXPANDED_VBOX-V-T
    DATA(context_m_expanded_vbox) = card_top->vbox( class = 'RAKEGA-context-m-expanded-vbox' ).
    DATA(context1_me_cont) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx1_me_label) = context1_me_cont->label( text = get_text_by_id( 'TEN_TITLEDEED' ) class = 'Body_1_3 color-gray7' ).
    DATA(ctx1_me_value) = context1_me_cont->label( text = '{TITLE_DEED3}' class = 'Body_1_3 color-gray7' ).
    DATA(context2_me_cont) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx2_me_label) = context2_me_cont->label( text = get_text_by_id( 'TEN_BUILDINGID' ) class = 'Body_1_3 color-gray7' ).
    DATA(ctx2_me_value) = context2_me_cont->label( text = '{BUILDING_ID}' class = 'Body_1_3 color-gray7' ).
    DATA(context3_me_cont) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-cont' ).
    " TODO(CTX3_ME_LABEL): no VALUE and no TECHNICAL_NAME in source - binding unknown
    DATA(ctx3_me_label) = context3_me_cont->label( text = '' class = 'Body_1_3 color-gray7' ).
    " AMBIGUOUS(CTX3_ME_VALUE): VALUE='Residential' also set in source (ignored here, using TECHNICAL_NAME binding) - see note below
    DATA(ctx3_me_value) = context3_me_cont->label( text = '{BUILDING_TYPE}' class = 'Body_1_3 color-gray7' ).
    DATA(context4_me_cont) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx4_me_value) = context4_me_cont->label( text = '{FLOORS}' class = 'Body_1_3 color-gray7' ).
    DATA(ctx4_me_label) = context4_me_cont->label( text = get_text_by_id( 'TEN_FLOORS' ) class = 'Body_1_3 color-gray7' ).
    DATA(context5_me_cont) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-cont' ).
    DATA(ctx5_me_value) = context5_me_cont->label( text = '{UNITS}' class = 'Body_1_3 color-gray7' ).
    DATA(ctx5_me_label) = context5_me_cont->label( text = get_text_by_id( 'TEN_UNITS' ) class = 'Body_1_3 color-gray7' ).
    DATA(ctx_less_hbox) = context_m_expanded_vbox->hbox( class = 'RAKEGA-context-less-hbox' justifycontent = 'End' ).
    DATA(ctx_less) = ctx_less_hbox->button( id = 'CTX_LESS' text = get_text_by_id( 'LESS_DETAILS' ) class = 'RAKEGA-context-m-button' type = 'Transparent' ).
    " TODO(CTX_LESS): no EVENT in source - this button drives a client-side show/hide chain instead (UI_FIELD_LOGICS): CONTEXT_M_COLLAPSED_HBOX-V-T,CONTEXT_M_EXPANDED_VBOX-V-F
    DATA(description_details_cont) = card_top->vbox( class = 'RAKEGA-card-descriptioncont RAKEGA-hide-in-mobile sapUiSmallMarginTop' alignitems = 'Center' ).
    ...
    DATA(journey_description_mobile) = description_details_panel->text_area( value = get_text_by_id( 'DESCRIPTION_RA_SELECTPARCELORGRANTED_1_1' ) maxlength = '65' class = 'Body_1_3 color-gray7 RAKEGA-lobby-ex-text' ).
    DATA(part2) = card->vbox( class = 'RAKEGA-part2' ).
    DATA(title) = part2->label( text = get_text_by_id( 'NNTC_1_6_TITLE' ) class = 'H3_1 color-gray7' ).
    DATA(main_vbox) = part2->vbox( class = 'NNTC_1_6-main-vbox' ).
    DATA(line1) = main_vbox->hbox( class = 'NNTC_1_6-line' ).
    DATA(vbox_1_1) = line1->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_1_1) = vbox_1_1->label( text = get_text_by_id( 'TEN_PLOTNUMBER' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_1_1) = vbox_1_1->input( value = '{PLOT_NUMBER}' class = 'input' ).
    DATA(vbox_1_2) = line1->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_1_2) = vbox_1_2->label( text = get_text_by_id( 'TEN_BUILDINGNUMBER' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_1_2) = vbox_1_2->input( value = '{BUILDING_NUMBER}' class = 'input' ).
    DATA(line1_1) = main_vbox->hbox( class = 'NNTC_1_6-line' ).
    DATA(vbox_1_4) = line1_1->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_1_4) = vbox_1_4->label( text = get_text_by_id( 'TEN_OWNERSHIPTYPE' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_1_4) = vbox_1_4->input( value = '{OWNERSHIP_TYPE}' class = 'input' ).
    DATA(line2) = main_vbox->hbox( class = 'NNTC_1_6-line' ).
    DATA(vbox_2_1) = line2->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_2_1) = vbox_2_1->label( text = get_text_by_id( 'TEN_UNITTYPE' ) class = 'Body_1_2 color-gray7' ).
    DATA(combobox_2_1) = vbox_2_1->combo_box( selectedkey = '{UNIT_TYPE}' placeholder = get_text_by_id( 'SELECT' ) class = 'combobox' ).
    " TODO(COMBOBOX_2_1): items come from search-help ZSH_CJ_TEN_UNIT_TYPE - value-list binding not yet confirmed
    DATA(vbox_2_2) = line2->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_2_2) = vbox_2_2->label( text = get_text_by_id( 'TEN_FLOORNUMBER' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_2_2) = vbox_2_2->input( value = '{FLOOR_NUMBER}' class = 'input' ).
    DATA(vbox_1_3) = line2->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_1_3) = vbox_1_3->label( text = get_text_by_id( 'TEN_UNITNUMBER' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_1_3) = vbox_1_3->input( value = '{UNIT_NUMBER}' class = 'input' ).
    DATA(line2_2) = main_vbox->hbox( class = 'NNTC_1_6-line' ).
    DATA(vbox_2_4) = line2_2->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_2_4) = vbox_2_4->label( text = get_text_by_id( 'TEN_UNITAREA' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_2_4) = vbox_2_4->input( value = '{UNIT_AREA}' type = 'Number' class = 'input' ).
    DATA(vbox_2_3) = line2_2->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_2_3) = vbox_2_3->label( text = get_text_by_id( 'TEN_NUMOFROOMS' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_2_3) = vbox_2_3->input( value = '{NUMBER_OF_ROOMS}' type = 'Number' class = 'input' ).
    DATA(vbox_2_3_a) = line2_2->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_2_3_a) = vbox_2_3_a->label( text = get_text_by_id( 'TEN_NUMOFDOORS' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_2_3_a) = vbox_2_3_a->input( value = '{NUMBER_OF_DOORS}' type = 'Number' class = 'input' ).
    DATA(line3) = main_vbox->hbox( class = 'NNTC_1_6-line' ).
    DATA(vbox_3_0) = line3->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_3_0) = vbox_3_0->label( text = get_text_by_id( 'TEN_EWETYPE' ) class = 'Body_1_2 color-gray7' ).
    DATA(combobox_3_0) = vbox_3_0->combo_box( selectedkey = '{EWE_TYPE}' placeholder = get_text_by_id( 'SELECT' ) class = 'combobox' ).
    " TODO(COMBOBOX_3_0): items come from search-help ZSH_CJ_TEN_EWE_TYPE - value-list binding not yet confirmed
    DATA(vbox_3_1) = line3->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_3_1) = vbox_3_1->label( text = get_text_by_id( 'TEN_EWEACCNUM' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_3_1) = vbox_3_1->input( value = '{EWE_ACC_NUMBER}' type = 'Number' class = 'input' ).
    DATA(vbox_3_2) = line3->vbox( class = 'NNTC_1_6-inner-vbox' ).
    DATA(label_3_2) = vbox_3_2->label( text = get_text_by_id( 'TEN_EWEPREMISENUM' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_3_2) = vbox_3_2->input( value = '{EWE_PREMISE_NUMBER}' type = 'Number' class = 'input' ).
    DATA(line4) = main_vbox->hbox( class = 'NNTC_1_6-line' ).
    DATA(vbox_3_3) = line4->vbox( class = 'NNTC_1_6-location-vbox' ).
    DATA(label_3_3) = vbox_3_3->label( text = get_text_by_id( 'TEN_LOCATION' ) class = 'Body_1_2 color-gray7' ).
    DATA(input_3_3) = vbox_3_3->input( value = '{LOCATION}' class = 'input' ).
    DATA(footer) = firstcontainer->hbox( class = 'RAKEGA-footer' ).
    DATA(buttonback) = footer->button( id = 'BUTTONBACK' text = get_text_by_id( 'BACK_BUTTON' ) class = 'regularBTN_with_border' icon = 'sap-icon://icomoon/Left' press = client->_event( 'BACK' ) ).
    DATA(next) = footer->button( id = 'NEXT' text = get_text_by_id( 'NEXT_BUTTON' ) class = 'regularBTN' icon = 'sap-icon://icomoon/Right' press = client->_event( 'SAVE' ) ).
