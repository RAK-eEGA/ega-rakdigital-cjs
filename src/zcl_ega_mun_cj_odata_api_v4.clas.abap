CLASS zcl_ega_mun_cj_odata_api_v4 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        !partner TYPE /iwbep/t_cod_select_options OPTIONAL
        !role    TYPE /iwbep/t_cod_select_options OPTIONAL
    ...
    METHODS get_deed_type
      IMPORTING
        VALUE(intreno)    TYPE recaintreno
      RETURNING
        VALUE(deed_type)  TYPE string .
    CLASS-METHODS get_building
      IMPORTING
        !aoid         TYPE rebdaoid
      EXPORTING
...
CLASS zcl_ega_mun_cj_odata_api_v4 IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    IF deed IS NOT INITIAL.
      ...
      IF line_exists( properties[ role = 'TR0800' ] ).
        ...
        ELSE.
          ...
          DATA(lv_ownfound) = xsdbool( sy-subrc = 0 ).
          LOOP AT properties ASSIGNING <fs_property> WHERE role = 'TR0800'.
            <fs_property>-ownmethod = lv_ownmthd.
            IF lv_ownfound = abap_true.
    ...
    ELSE.
      ...
      IF line_exists( role[ low = 'ZLESOR' ] ).
        SELECT partner1 AS partner2
          FROM but050
          WHERE partner2 IN @partner AND reltyp = 'Y00100'
            AND date_from <= @sy-datum AND date_to >= @sy-datum
          INTO TABLE @lt_partner.
      ENDIF.

      LOOP AT partner INTO DATA(ls_partner).
        APPEND VALUE #( partner2 = ls_partner-low ) TO lt_partner.
      ENDLOOP.

      IF lt_partner IS INITIAL.
        RETURN.
      ENDIF.
      ...
      IF role IS INITIAL.
        SELECT a~intreno, a~objnr, a~aoid, b~partner, b~role, b~validfrom, b~validto
          ...
          WHERE b~partner = @lt_partner-partner2
            AND b~role IN ( 'TR0800' , 'ZLESOR' , 'ZINVES' )
            AND b~validfrom <= @sy-datum AND b~validto >= @sy-datum
          INTO TABLE @lt_ao.
      ELSE.
        SELECT a~intreno, a~objnr, a~aoid, b~partner, b~role, b~validfrom, b~validto
      ...
      IF lt_lrkey IS NOT INITIAL.
        ...
        LOOP AT lt_lr INTO DATA(ls_lr2).
          ...
          IF sy-subrc <> 0.
            CONTINUE.
          ENDIF.
          <fs_property>-ownmethod = ls_lrown-zzown_mth.
      ...
      IF lt_gobj IS NOT INITIAL.
        ...
        SELECT b~objnrtrg, a~recnendabs
          ...
          WHERE b~objnrtrg = @lt_gobj-objnr
            ...
            AND c~role = 'ZTR080' AND c~partner IN @partner
            AND c~appl = '0036'
  ...
  METHOD get_ao_header.
    ...
    SELECT a~intreno, a~aoid, a~xao, a~validfrom, a~validto,
           b~aotype AS xmaotype, c~xmaofunction,
  ...
  METHOD get_deed_type.

    DATA: parent TYPE recaintreno,
          count  TYPE i.

    parent = VALUE #( properties[ intreno = intreno ]-intreno_p OPTIONAL ).

    LOOP AT properties TRANSPORTING NO FIELDS WHERE intreno_p = parent.
      count = count + 1.
      IF count > 1.
        EXIT.
      ENDIF.
    ...
    deed_type = COND #( WHEN count = 1 THEN '2' ELSE '1' ).
