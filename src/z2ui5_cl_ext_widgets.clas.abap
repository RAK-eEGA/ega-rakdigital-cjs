  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_raksearch_methods,
        ...
        deed          TYPE flag,
      END OF ty_raksearch_methods .
    TYPES:
      tt_raksearch_methods TYPE STANDARD TABLE OF ty_raksearch_methods WITH DEFAULT KEY .
    TYPES:
      BEGIN OF ty_raksearch,
        id            TYPE string,
        method        TYPE string,
        mask_input    TYPE string,
        input         TYPE string,
        date          TYPE char8,
        passport_type TYPE dd07t-domvalue_l,
        nationality   TYPE t005t-land1,
        ...
        label4        TYPE string,
        value4        TYPE string,
        methods       TYPE tt_raksearch_methods,
      END OF ty_raksearch .
    TYPES:
...
*'}'.
  ...
  METHOD raksearch.
    ...
    DATA: lv_index   TYPE char1,
          lv_visible TYPE char5.

    me->raksearch_functions( ).

    READ TABLE mt_raksearch INTO DATA(ls_search) WITH KEY id = search_id.
    CHECK sy-subrc EQ 0.
    DATA(lv_tabix) = sy-tabix.
    SUBTRACT 1 FROM lv_tabix.
    lv_index = lv_tabix.

    DATA(search) = io_parent->hbox( alignitems = 'End' visible = '{= !${/XX/MT_RAKSEARCH/' && lv_index && '/FOUND}}' ).
    IF lines( ls_search-methods ) > 1.
      lv_visible = 'true'.
      READ TABLE mt_raksearch ASSIGNING FIELD-SYMBOL(<search>) WITH KEY id = search_id.
      IF sy-subrc EQ 0.
        CLEAR: <search>-method.
      ENDIF.
    ELSE.
      lv_visible = 'false'.
      READ TABLE ls_search-methods INTO DATA(ls_method) INDEX 1.
      READ TABLE mt_raksearch ASSIGNING <search> WITH KEY id = search_id.
      IF sy-subrc EQ 0.
        <search>-method = ls_method-method.
      ENDIF.
    ENDIF.
    DATA(select_vbox) = search->vbox( class = 'sapUiSmallMarginEnd' visible = lv_visible ).
    select_vbox->label( text = 'Search By'(102) required = 'true' class = 'Body_1_2 color-gray7' ).
    DATA(select) = select_vbox->combo_box( items = '{/XX/MT_RAKSEARCH/' && lv_index && '/METHODS}' enabled = 'true'
    selectedkey = '{/XX/MT_RAKSEARCH/' && lv_index && '/METHOD}'
    placeholder = ' - Select - ' change = '.onRAKSearchChange' class = 'combobox' )->ui_core_item( key = '{METHOD}' text = '{DESCRIPTION}' ).


    IF ls_method-input EQ abap_true AND ls_method-mask IS NOT INITIAL.
      lv_visible = 'true'.
    ELSE.
      lv_visible = 'false'.
    ENDIF.
    DATA(vbox_1) = search->vbox( class = 'sapUiSmallMarginEnd' visible = lv_visible ).
    vbox_1->label( text = '' required = 'true' class = 'Body_1_2 color-gray7' ).
    vbox_1->mask_input( value = '{/XX/MT_RAKSEARCH/' && lv_index && '/MASK_INPUT}' class = 'input' ).

    IF ls_method-input EQ abap_true AND ls_method-mask IS INITIAL.
      lv_visible = 'true'.
    ELSE.
      lv_visible = 'false'.
    ENDIF.
    DATA(vbox_2) = search->vbox( class = 'sapUiSmallMarginEnd' visible = lv_visible ).
    vbox_2->label( text = '' required = 'true' class = 'Body_1_2 color-gray7' ).
    vbox_2->input( value = '{/XX/MT_RAKSEARCH/' && lv_index && '/INPUT}' visible = 'true' class = 'input' ).

    IF ls_method-birth_date EQ abap_true.
      lv_visible = 'true'.
    ELSE.
      lv_visible = 'false'.
    ENDIF.
    DATA(vbox_3) = search->vbox( class = 'sapUiSmallMarginEnd' visible = lv_visible ).
    vbox_3->label( text = 'Date of Birth'(103) required = 'true' class = 'Body_1_2 color-gray7' ).
    vbox_3->date_picker( value = '{ path:"/XX/MT_RAKSEARCH/' && lv_index && '/DATE", type:"sap.ui.model.type.Date", formatOptions:{ source:{ pattern:"yyyyMMdd"}, pattern:"dd/MM/yyyy"}}'
    displayformat = 'dd/MM/yyyy'
    placeholder = 'dd/MM/yyyy'
*    valueformat = 'yyyyMMdd'
    class = 'datepicker editable-datepicker' ).

    IF ls_method-passport_type EQ abap_true.
      lv_visible = 'true'.
    ELSE.
      lv_visible = 'false'.
    ENDIF.
    DATA(vbox_4) = search->vbox( class = 'sapUiSmallMarginEnd' visible = lv_visible ).
    vbox_4->label( text = 'Passport Type'(104) required = 'true' class = 'Body_1_2 color-gray7' ).
    DATA(passport_type) = vbox_4->combo_box( selectedkey = '{/XX/MT_RAKSEARCH/' && lv_index && '/PASSPORT_TYPE}' class = 'combobox' placeholder = ' - Select - ' ).
    ...
    IF ls_method-nationality EQ abap_true.
      lv_visible = 'true'.
    ELSE.
      lv_visible = 'false'.
    ENDIF.
    DATA(vbox_5) = search->vbox( class = 'sapUiSmallMarginEnd' visible = lv_visible ).
    vbox_5->label( text = 'Nationality'(105) required = 'true' class = 'Body_1_2 color-gray7' ).
    DATA(nationality) = vbox_5->combo_box( selectedkey = '{/XX/MT_RAKSEARCH/' && lv_index && '/NATIONALITY}' class = 'combobox' placeholder = ' - Select - ' ).
    ...
    IF ls_method-deed EQ abap_true.
      lv_visible = 'true'.
    ELSE.
      lv_visible = 'false'.
    ENDIF.
    DATA(vbox_6) = search->vbox( class = 'sapUiSmallMarginEnd' visible = lv_visible ).
    DATA(titledeed_label) = vbox_6->label( text = 'Title Deed'(106) class = 'Body_1_2 color-gray7' required = 'true' ).

    DATA(input_cont) = vbox_6->hbox( rendertype = 'Bare' alignitems = 'Center' ).
    DATA(number_input) = input_cont->input( value = '{/XX/MT_RAKSEARCH/' && lv_index && '/DEED}' maxlength = '5' type = 'Number' placeholder = 'Number'(107) class = 'input' ).
    DATA(slash_text) = input_cont->text( text = '/' class = 'Body_2_3 color-gray7 sapUiTinyMarginBeginEnd' ).
    DATA(year_input) = input_cont->input( value = '{/XX/MT_RAKSEARCH/' && lv_index && '/YEAR}' maxlength = '4' width = '6rem' placeholder = 'Year'(108) class = 'input' ).
    DATA(vbox_end) = search->vbox( height = '100%' alignitems = 'End' justifycontent = 'End' )->button( class = 'regularBTN_with_border'
    visible = '{= ${/XX/MT_RAKSEARCH/' && lv_index && '/METHOD} !== ""}'
    text = 'Search'(109) press = client->_event( val = 'SEARCH' t_arg = VALUE #( ( search_id ) ) ) ).

    DATA(results) = io_parent->hbox( justifycontent = 'SpaceBetween' visible = '{/XX/MT_RAKSEARCH/' && lv_index && '/FOUND}' ).
    DATA(result_cont) = results->hbox( ).
    DATA(col1) = result_cont->vbox( class = 'sapUiSmallMarginBeginEnd' ).
    DATA(label1) = col1->text( text = '{/XX/MT_RAKSEARCH/' && lv_index && '/LABEL1}' class = 'Body_1_2 color-gray7' ).
    DATA(value1) = col1->text( text = '{/XX/MT_RAKSEARCH/' && lv_index && '/VALUE1}' class = 'Body_2_3 color-gray6' ).
    DATA(col2) = result_cont->vbox( class = 'sapUiSmallMarginBeginEnd' ).
    DATA(label2) = col2->text( text = '{/XX/MT_RAKSEARCH/' && lv_index && '/LABEL2}' class = 'Body_1_2 color-gray7' ).
    DATA(value2) = col2->text( text = '{/XX/MT_RAKSEARCH/' && lv_index && '/VALUE2}' class = 'Body_2_3 color-gray6' ).
    DATA(col3) = result_cont->vbox( class = 'sapUiSmallMarginBeginEnd' ).
    DATA(label3) = col3->text( text = '{/XX/MT_RAKSEARCH/' && lv_index && '/LABEL3}' class = 'Body_1_2 color-gray7' ).
    DATA(value3) = col3->text( text = '{/XX/MT_RAKSEARCH/' && lv_index && '/VALUE3}' class = 'Body_2_3 color-gray6' ).
    DATA(col4) = result_cont->vbox( class = 'sapUiSmallMarginBeginEnd' ).
    DATA(label4) = col4->text( text = '{/XX/MT_RAKSEARCH/' && lv_index && '/LABEL4}' class = 'Body_1_2 color-gray7' ).
    DATA(value4) = col4->text( text = '{/XX/MT_RAKSEARCH/' && lv_index && '/VALUE4}' class = 'Body_2_3 color-gray6' ).
    DATA(delete) = results->icon( src = 'sap-icon://icomoon/Delete' press = '.onDeleteSearchResults($event, ''/XX/MT_RAKSEARCH/' && lv_index && '/FOUND'')' class = 'font-1d5rem' ).
...
'         controls[6].setVisible(false);' && |\n| &&
'       if (item.INPUT && item.MASK !== ""){' && |\n| &&
...
'       }' && |\n| &&
'       if (item.DEED){' && |\n| &&
'         controls[6].setVisible(true);' && |\n| &&
'       }' && |\n| &&
'     }    '.
    me->add_function( ev_function ).

    ev_function =
'   onDeleteSearchResults(oEvent, itemPath){' && |\n| &&
'     oEvent.getSource().getModel().setProperty(itemPath, false);' && |\n| &&
'     oEvent.getSource().getModel().refresh(false);' && |\n| &&
'   }'.
    me->add_function( ev_function ).
  ...
  METHOD raksearch_get_data.

    DATA: lr_building  TYPE RANGE OF vibdao-aotype.
...
*    <search>-year   = '2025'.
    ...
    CASE <search>-method.
      ...
      WHEN 'TRADE_LICENSE'.
      WHEN 'DEED'.
        IF <search>-deed IS NOT INITIAL AND <search>-year IS NOT INITIAL.
          DATA(lv_year) = <search>-year.
...
*          SELECT vilmlr~intreno, vilmlr~objnr,
...
*            vilmlr~zzown_mth, zdt_refx_ownmtt~description AS property_type,
*            vibdao~intreno AS ao_intreno ,vibdao~aoid, vibdao~aotype
*            FROM vilmlr
*            INNER JOIN vilmrg
*            ON  vilmrg~intreno EQ vilmlr~intreno
*            INNER JOIN jest
*            ON  jest~objnr EQ vilmrg~plobjnr
*            INNER JOIN zdt_refx_objas
*            ON  zdt_refx_objas~objnrsrc EQ vilmlr~objnr
*            INNER JOIN vibdao
*            ON  vibdao~objnr     EQ zdt_refx_objas~objnrtrg
*            AND vibdao~validfrom LE @sy-datum
*            AND vibdao~validto   GE @sy-datum
*            LEFT OUTER JOIN vibdcharact
*            ON  vibdcharact~intreno       EQ vilmlr~intreno
...
*            LEFT OUTER JOIN tivbdcharactt
*            ON  vibdcharact~fixfitcharact EQ tivbdcharactt~fixfitcharact
*            AND tivbdcharactt~spras       EQ @sy-langu
*            LEFT OUTER JOIN tivajrlrasrch
*            ON  vibdcharact~fixfitcharact EQ tivajrlrasrch~fixfitcharact
*            AND tivajrlrasrch~rlragrpchct EQ 'USAGE'
*            LEFT OUTER JOIN tivajchctgroupt
*            ON tivajrlrasrch~rlragrpchct  EQ tivajchctgroupt~rlragrpchct
*            AND tivajchctgroupt~spras     EQ @sy-langu
*            LEFT OUTER JOIN vibdlochier
*            ON  vibdlochier~intreno       EQ vilmlr~intreno
*            AND vibdlochier~locsys        EQ 'RAK'
*            LEFT OUTER JOIN tivbdlochier
*            ON  tivbdlochier~locsys       EQ vibdlochier~locsys
*            AND tivbdlochier~lochier      EQ vibdlochier~lochier
*            LEFT OUTER JOIN zdt_refx_ownmtt
*            ON  zdt_refx_ownmtt~own_mth   EQ vilmlr~zzown_mth
*            AND zdt_refx_ownmtt~spras     EQ @sy-langu
*            INTO TABLE @DATA(lt_deedo)
*            WHERE vilmlr~lrvolumeno       EQ @<search>-deed
*            AND   vilmlr~lrpageno         EQ @lv_year
*            AND   jest~stat               IN ( 'E0011' , 'E0012' , 'E0013' )
*            AND   jest~inact              EQ @abap_false
*            ORDER BY vilmlr~intreno.

          <search>-found = abap_true.
          <search>-label1 = 'sdfdsfdsf'.
          <search>-value1 = 'dfgfdgfdg'.
          <search>-label2 = 'sdfdsfdsf'.
          <search>-value2 = 'dfgfdgfdg'.
          <search>-label3 = 'sdfdsfdsf'.
          <search>-value3 = 'dfgfdgfdg'.
          <search>-label4 = 'sdfdsfdsf'.
          <search>-value4 = 'dfgfdgfdg'.
        ENDIF.
    ENDCASE.
  ENDMETHOD.
ENDCLASS.
